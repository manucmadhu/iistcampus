...
def lagrange(t, col3, col10):
    ...
    if t in col3:
        ...
        return flux
def quad_lagrange(t, col3, col10):
    # If t exactly matches a known point
    if t in col3:
        ind = np.where(col3 == t)[0][0]
        return float(col10[ind])
...
for t in time:
    ...
    flux1=quad_lagrange(t,col3,col10)
    flux_range.append(flux)
    flux_quad.append(flux1)
plt.plot(time,flux_range,label="Lagrange Linear")
plt.plot(time,flux_quad,label="Lagrange Quadratic")
flux_interp = np.interp(time, col3, col10)
plt.plot(time,flux_interp,label="Numpy interpolation")
